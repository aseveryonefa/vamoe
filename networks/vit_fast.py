...
from .dit_moe import PositionEmbedder, ChannelMoeBlock
# from moe_layer import Mlp, MoE
# from dit_moe import PositionEmbedder, ChannelMoeBlock
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False,
                 ...
                 use_moe='densemoe', num_exports=69, noisy_gating=True, k_element=2, allow_print=True, topk=2):
        super().__init__()
        ...
        if self.use_moe=='channelmoe':
            if allow_print:   logging.info('****** using ChannelMoeBlock!!! ******')
            self.moe = ChannelMoeBlock(embed_dim=dim, mlp_ratio=mlp_ratio, num_experts=num_exports, topk=self.topk)
        
   

    def forward(self, x, H, W, posembed=None, mask=None):
        x0 = x + self.drop_path(self.attn(self.norm1(x), H, W))
        x = self.norm2(x0)
        ...
        if self.use_moe=='moe':
            ...
            x = x.view(-1, self.dim)
            x, loss = self.moe(x)
            # print('moe output:', x.shape)
            x = x.view(-1, H*W, self.dim)
        elif self.use_moe=='densemoe' or self.use_moe=='channelmoe' or self.use_moe=='channelmoev1':
            x = self.moe(x, posembed)
        elif self.use_moe=='channelmoev3':
            x, expert_output = self.moe(x, posembed)
        else:
            x = self.mlp(x)
...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, patch_stride=16, in_chans=3, embed_dim=768, area_equal_patch=False):
        super().__init__()
...
class ViT(BaseModel):
    ...
    def __init__(self, img_size=224, patch_size=16, patch_stride=16, in_chans=3, out_chans=227, num_classes=80, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, window_size=(14,14),
                 drop_path_rate=0., norm_layer=None, window=True,area_equal_patch=False,
        ...
        self.area_equal_patch=area_equal_patch
        global COMPAT
        COMPAT = compat
        ...
        self.patch_embed = PatchEmbed(
            ...
            in_chans=in_chans, embed_dim=embed_dim, area_equal_patch=area_equal_patch)
        ...
        if use_abs_pos_emb:
            ...
            if self.area_equal_patch:
                self.pos_embed.data.copy_(torch.from_numpy(pos_embed[:num_patches]).float().unsqueeze(0))
            else:
        ...
        for i in range(depth):
            block = Block(
                ...
                rel_pos_spatial=rel_pos_spatial, use_moe=False,
                act_layer=QuickGELU if act_layer == 'QuickGELU' else nn.GELU
            )
            # if lms_checkpoint_train :
                # import pdb
                # pdb.set_trace()
                # block = checkpoint_sequential(block, offload_to_cpu=True)
            self.blocks.append(block)

        self.ln_pre = norm_layer(embed_dim) if pre_ln else nn.Identity()  # for clip model only
        self.norm = norm_layer(embed_dim)

        if self.img_size==(721, 1440) or img_size==(161,161):
    ...
    def embedding_forward(self, x, mask=None, **kwargs):
        ...
        if self.time_embed:
            ...
            hour=None
            month=None
        # if time_hour is not None and self.time_embed:
        #     day_embed = self.hour_embed(time_hour).view(B, C, H, W)
        #
        #     month_embed =  self.month_embed(time_month).view(B, C, H, W)
        # #
        #     x1, (Hp, Wp), mask = self.patch_embed_time(day_embed+month_embed, mask)
            # x = x+day_embed+month_embed
        self.patch_embed = self.patch_embed.to(torch.float32)
        ...
        if self.test_pos_mode is False:
            if x.size(1) == self.pos_embed.size(1):
                ...
                x = x + self.pos_embed.reshape(1, self.patch_embed.patch_shape[0],
                                               ...
                                               self.pos_embed.size(2))[:,:self.Hp, :self.Wp, :].reshape(1, x.size(1),
            ...
            orig_size = (math.ceil(self.Hp/20)*7, math.ceil(self.Wp/20)*7)
            ...
            if self.time_embed:
                pos_embed = torch.einsum("bn,njk->bjk", [hour,self.pos_embed])
            else:
                pos_embed = self.pos_embed
            # import  pdb
            # pdb.set_trace()
            x = x + pos_embed     #get_abs_pos(pos_embed, False, (self.ori_Hp, self.ori_Hw), patch_shape)
        else:
            raise NotImplementedError

        # x = self.random_masking(x) # effective only if self.mask_input=True (default False), for mask based ssl
        return x
    def features_forward(self, x, return_layer_fea=False):
    ...
    def decoder_forward(self, x):
        self.final = self.final.to(torch.float32)
        ...
        if self.area_equal_patch:
            ...
            if self.img_size==(721, 1440) or self.img_size==(161, 161):
    ...
    def forward(self, input_var, **kwargs):
        ...
        if kwargs.get('only_return_embeding', False):
            embed = x
            # x, fea = self.features_forward(x, return_layer_fea=True)
            # import pdb
            # pdb.set_trace()
            return embed 
            # return torch.cat([embed, fea], dim=1)
        
        if kwargs.get('return_embeding_output', False):
            embed = x
            x = self.features_forward(x, return_layer_fea=False)

            output = self.decoder_forward(x)
            # import pdb
            # pdb.set_trace()
            return output, embed #torch.cat([embed,fea], dim=1)
        else:
            x = self.features_forward(x)

            # print('encoding output:', x.shape)
...
class VitFastBackbone(BaseModel):
    def __init__(self, arch='vit_base', patch_size=(16,16),patch_stride=None, in_chans=227, out_chans=227,
                 pretrained_model=None, finetune_model=None,prompt=None, kwargs=None):
        super().__init__()
        # import yaml
        # # f = open('./backbones/maevit/config.yaml')
        # # cfg = f.read()
        # # net_configer = yaml.load(cfg, Loader=yaml.FullLoader)
        # # net_configer = net_configer['common']['backbone']['kwargs']
        if patch_stride is None:
            patch_stride =patch_size
        
        tiny_default_dict =dict(
            drop_path_rate=0, use_abs_pos_emb=True,  # as in table 11
            patch_size=patch_size, patch_stride=patch_stride, in_chans=in_chans, out_chans=out_chans, embed_dim=512, depth=12,
            num_heads=8, mlp_ratio=4, qkv_bias=True,  prompt =prompt,
        ...
        base_default_dict =dict(
            ...
            num_heads=12, mlp_ratio=4, qkv_bias=True,  prompt =prompt,
            norm_layer=partial(nn.LayerNorm, eps=1e-6),
            # learnable_pos= True,
        )
        ...
        large_default_dict =dict(
            ...
            num_heads=16, mlp_ratio=4, qkv_bias=True, prompt =prompt,
            norm_layer=partial(nn.LayerNorm, eps=1e-6),
            # learnable_pos= True,
        )
...
if __name__ == '__main__':
    ...
    print('end')
    # model = MAEvitBackbone(arch=arch,patch_size=patch_size, patch_stride=patch_stride, in_chans=in_chans,
    #                        out_chans=out_chans, pretrained_model=pretrained_model, kwargs=kwargs)
    model = VitFastBackbone(arch=arch, patch_size=patch_size, patch_stride=patch_stride, in_chans=in_chans, out_chans=out_chans,
                 pretrained_model=pretrained_model, kwargs=kwargs)
    ...
    input = torch.randn(1, 65, 128, 256).cuda()
